...
function Header_nav({ content }) {
    ...
    function Topbar({ setMenuStatus }) {
        return (
            <div
                className={cx('Main_nav_controller')}
                onClick={() => {
                    if (iscolor === false) {
                        setcolor(true);
                    } else {
            ...
            >
                <div>
                    <FontAwesomeIcon icon={faNoteSticky} style={{ color: iscolor ? 'white' : '' }} />
                </div>
                <div className={cx('Main_nav_controller2')} style={{ color: iscolor ? 'white' : '' }}>
                    Dashboards
                </div>
                <div>
                    <FontAwesomeIcon icon={faChevronDown} style={{ color: iscolor ? 'white' : '' }} />
                </div>
            </div>
        );
    }
